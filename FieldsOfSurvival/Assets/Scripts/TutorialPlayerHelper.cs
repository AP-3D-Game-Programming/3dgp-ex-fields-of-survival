...
public class TutorialPlayerHelper : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        switch (tutorial.step)
        {
            ...
            case 1: // detect shooting
                if (Mouse.current.leftButton.wasPressedThisFrame)
                    tutorial.NextStep();
                break;

            case 2: // detect plant/harvest
                if (Keyboard.current.fKey.wasPressedThisFrame || Keyboard.current.hKey.wasPressedThisFrame)
                    tutorial.NextStep();
                break;
        }
    }
